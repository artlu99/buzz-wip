...
export interface DeleteMessage {
	...
	networkMessageId: string; // Network message ID from the original message, used for matching across distributed stores
	channelName: string; // plaintext
	deletedBy: string; // the user deleting the message (should be the original sender)
}
...
export const TIMESTAMP_TOLERANCE_MS = 30000; // 30 seconds tolerance for message matching (accounts for network latency and clock skew)
...
export class TypedWsClient {
	...
	private getMessageKey(wsMessage: WsMessage): string | null {
		...
		if (isReactionMessage(message)) {
			...
			return `reaction:${message.networkMessageId}:${message.createdBy}:${message.reaction}:${message.isDeleted}:${roundedTime}`;
		}
		...
		if (isDeleteMessage(message)) {
			const roundedTime =
				Math.floor(date / TIME_ROUNDING_MS) * TIME_ROUNDING_MS;
			return `delete:${message.networkMessageId}:${message.deletedBy}:${roundedTime}`;
		}
